...
If Badge finds `.badge-local/ports.json` from the superseded two-origin launcher, `npm start` preserves it and stops. Run `npm run recover:legacy` to reopen Archive and Studio at those exact old origins without creating or changing `.badge-local/site.json`; the in-app links also stay pinned to that recorded pair. Recovery startup is serialized so overlapping commands cannot split the pair. Archive can then export a backup. Studio backup is not implemented yet, so keep the legacy record and use recovery mode for any Studio work that matters; do not delete the record and strand that old-origin IndexedDB data.
