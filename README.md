...
The repository now contains a runnable first vertical slice with separately built Badge Archive and Badge Studio applications served as one local website, shared strict contracts, browser-local persistence, a searchable Archive Discover catalogue for all `66` unique visualized badge concepts, preselected historical quotations with explicit live Claude Code regeneration, deterministic fixture art, pack compilation and admission, and an interactive 3D renderer with a static fallback.
