...
export const selectedBookStudies = bindSelectedStudies(
  ...
  "books-read",
  (record) => {
    const style = findArtStyle(record.candidateStyles[0]);
    if (!style) {
      throw new Error(
        `Selected book study ${record.title} references missing primary style ${record.candidateStyles[0]}; register that immutable style revision before binding generated art.`,
      );
    }
    return `Selected ${style.label} badge study for ${record.title}: ${record.artBrief.description}`;
  },
);
...
export const selectedEducationMilestoneStudies = bindSelectedStudies(
  ...
  "life-milestones",
  (record) => record.accessibleDescription,
);
...
function bindSelectedStudies<T extends CatalogueStudyRecord>(
  ...
  return records.map((record) => {
    ...
    return {
      ...record,
      selectedSource: buildSelectedSource(record.slug, primary.candidateKey, describe(record), integrity),
    };
  });
}
...
function buildSelectedSource(
  ...
  const codeNativeRecipe = findCodeNativeArtRecipe(slug);
  ...
  return {
    ...
    promptSha256: integrity.canonicalPromptSha256,
    generatedOn: isDirectCodeNative ? "2026-08-26" : "2026-08-25",
    promptRecipe: { id: "badge-source-art", revision: 2 },
    sourceHistory,
