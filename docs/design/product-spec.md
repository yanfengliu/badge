...
Every definition exposed as a usable badge in Collection, detail, planning, activation, Timeline, or 3D inspection resolves through an installed published pack to one final presentation. Discover may additionally list and open a clearly labeled, read-only selected source study from the tracked safe catalogue projection; that preview is not a lifecycle state, installed definition, or activatable badge.
...
Discover searches and browses all `300` unique visualized badge concepts in tracked product inputs through five canonical sets: four published starter badges, sixty-two additional national-park studies after Yosemite deduplication, fifty U.S.-state studies, fifty book studies, two education-milestone studies, and `132` named Michelin-restaurant studies. The resulting set populations are U.S. National Parks `64`, U.S. States `50`, Books Read `51` including the published _Sapiens_ starter, Life Milestones `3` including the published bachelor's-degree starter, and Michelin Dining `132`; `4` concepts are published and `296` remain potential source studies. Every card is one full-surface native action. An earned published badge is full color and opens its exact memory replay; every unearned concept is grey. A published unearned badge enters preparation and activation without leaving Discover, while a source study remains labeled `Potential` and `Not yet published` and opens a read-only preview with its lazily loaded `384 × 384` detail derivative, criterion, location, set membership, restaurant cuisine where applicable, visual-basis description, and individual official reference where supplied. The source-study preview offers no activation, quotation, 3D, install, provider, authoring, or persistence action and falls back truthfully to the eager `128 × 128` thumbnail if its detail derivative fails. Search and set filtering apply to the complete projection, while the card grid renders an initial `24` matches and reveals subsequent batches only on explicit request. Selecting a set from memory replay opens that exact Discover set, and entering Discover from primary navigation clears that transient set context for independent browsing.
