...
Preparation traverses the exact ordered sequence currently exposed by Discover, including its set, region, and search filters; memory replay traverses only collected records in the browsed or primary set. Each surface places the visible previous, `x of y · context`, and next controls in a dedicated stable slot before title, quotation, note, and other badge-specific content. Within one viewport and sequence, the two `44 × 44px` step controls retain the same coordinates across title wrapping, index digit changes, source loading, lifecycle changes, and memory-body length; preparation entry normalizes the document to the top without skipping the pager in forward focus order, while replay exposes a named keyboard-focusable memory region and scrolls only that variable body so stepping never inherits an overlay scroll jump. If focus was on changing memory content, replay hands it to that visible region as the new body returns to the top rather than leaving an invisible focused action below the scroll position. Plain `ArrowLeft` and `ArrowRight` invoke the same steps while yielding to form controls and the explicitly engaged viewer, and sequence ends remain focusable but inert.
...
Before activation the surface shows the current quotation and two provenance rows plus one `Regenerate quote` action. `Historical figure` names the person and offers an optional English-Wikipedia biography link when the curated record has one; `Quote source` names the source title and always offers `View quote source` for the separately reviewed quotation evidence. The explicit regeneration action authorizes one latest-request-wins provider attempt and direct replacement only after the result is rebound to the originating shortlist, validated, and persisted. Disclosure, pending work, cancellation, stale completion, malformed output, provider failure, or persistence failure leaves the current quotation visible and unchanged; there is no proposal card, separate `Use this saying`, model-written original, or manual editor. The Archive repository rejects a canonically equivalent quotation already accepted by any other record inside the same update or activation transaction, so concurrent windows cannot both claim the same wording. Trusted values reserve their exact quotation text; raw legacy prose reserves the complete string; and ambiguous legacy formatted values reserve every plausible quotation prefix instead of guessing which attribution delimiter is authoritative. Activation consumes the already persisted source-checked quotation and atomically seals it with the earned memory; earned records show the quotation read-only and reject every later replacement attempt.
...
The `2026-08-27.badge-editorial-50@1` Video Games edition adds fifty finite completion achievements spanning historically broad platforms, eras, genres, and play relationships without asserting that the owner has completed any of them. Every game has three deterministic candidate directions, one selected source study and list derivative, and an honest game-specific criterion. The fifty primaries use all `24` registered authoring-direction labels through `14` realized code-native construction families, while the source art abstracts traversal, timing, building, cooperation, choice, rhythm, or discovery into simple broad geometry and excludes screenshots, key art, box art, characters, likenesses, signature props, UI, HUD, logos, wordmarks, franchise symbols, and game-specific trade dress. Accessible descriptions name that realized construction rather than falsely calling flat geometry watercolor, aquatint, or another authoring medium. Every record designates one exact source-checked Shakespeare quotation from _Hamlet_, Act II, Scene II, with separate quotation-source and English-Wikipedia biography provenance.
