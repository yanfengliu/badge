...
Discover searches and browses all `116` unique visualized badge concepts in tracked product inputs through four canonical sets: four published starter badges, sixty-two additional national-park studies after Yosemite deduplication, and fifty U.S.-state source studies. Every card is one full-surface native action. An earned published badge is full color and opens its exact memory replay; every unearned concept is grey. A published unearned badge enters preparation and activation without leaving Discover, while a source study remains labeled `Potential` and `Not yet published` and opens only a read-only preview of its admitted thumbnail, criterion, status, and set membership, with no activation, quotation, 3D, install, provider, authoring, or persistence action. Search and set filtering apply to the complete projection, while the card grid renders an initial `24` matches and reveals subsequent batches only on explicit request. Selecting a set from memory replay opens that exact Discover set, and entering Discover from primary navigation clears that transient set context for independent browsing.
