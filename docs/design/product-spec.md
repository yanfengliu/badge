...
The opening experience uses a warm field-cabinet treatment with one collapsed shelf per set, collected badge artifacts visible in full color, `x / y collected` as the only set count, collection search, and a small derived statistics strip for collected count, represented sets, occurrence-year span, and latest memory. Shelves start closed and expand to the complete earned records for that set without changing durable state.
...
Discover searches and browses all `66` unique visualized badge concepts in tracked product inputs through canonical sets: four published starter badges and sixty-two additional national-park studies, with Yosemite deduplicated in favor of its published entry. An earned published badge is full color and opens its exact memory replay; every unearned concept is grey. A published unearned badge may enter preparation and activation without leaving Discover, while a source study remains labeled `Potential` and `Not yet published` with no activation, quotation, 3D, install, provider, or authoring action. Selecting a set from memory replay opens that exact Discover set, and entering Discover from primary navigation clears that transient set context for independent browsing.
