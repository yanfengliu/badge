...
- Archive and Studio boot only as a complete site at their remembered durable origin, repeated startup reuses the exact `/` and `/studio/` applications already there, first selection can avoid an unrelated listener, verification cannot mutate canonical launcher state, and a preserved two-origin install can reopen both exact old origins with correct cross-origin navigation and serialized startup. Disposable fixtures remain isolated from durable data, schema upgrades and corrupt-store handling are tested, a later origin change cannot make data silently disappear, and no personal data enters Git.
