...
- Implement live Archive saying generation behind its separate provider-neutral port and separately capability-scoped companion when browser-safe authorization is unavailable, while preserving manual writing without it. Only explicit `Generate saying` and `Try another` actions call the port; page load, selection, activation, ceremony, reload, restore, and background work do not. Retrying copy cannot invoke Studio or art generation; disclose the exact minimal outbound text fields, use the versioned prompt and optional curated non-personal theme direction in `product-spec.md`, and exclude personal notes, dates, occurrences, accepted sayings, visibility, art, and unrelated draft state by default.
