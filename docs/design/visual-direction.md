...
The same direction makes single-badge surfaces traversable without turning navigation into moving content: preparation places a centered `‹ x of y · context ›` pager in the artifact pane's first stable row before the variable-height heading, and memory replay places the same pager in a fixed details slot before an independently scrollable memory body. Both step through their truthful sequences — preparation follows the exact ordered Discover grid including set, region, and search filters, while replay follows collected memories in the badge's browsed or primary set — and their fixed `44px` edge tracks keep both controls at identical coordinates as titles wrap, index digits change, sources load, or quotation and note length differ. Plain `ArrowLeft` and `ArrowRight` are keyboard equivalents on both surfaces; they yield to text fields and to the engaged 3D viewer, end-of-sequence steps stay focusable but inert instead of dropping focus, and a badge reached by paging renders its truthful state — an already-collected badge shows its earned panel, never a second activation form.
...
Badge Studio adjusts one badge, and its layout says so: the badge under adjustment on the left, the panels that change it on the right, and the badge's name in the header between the way out and the one save action. It identifies itself as Studio rather than the personal archive.
...
- [Stable preparation pager reference](assets/archive-preparation-pager-stable-2026-08-31.png): route `/#discover`, exact `1440 × 1000` browser viewport, fixture-only Played Prince of Persia preparation at `10 of 50` showing the pager in its stable leading slot before the two-line title and live artifact stage, captured 2026-08-31. This supersedes the former between-heading-and-stage placement in `archive-preparation-pager-2026-08-27.png`, which remains historical evidence.
- [Stable replay pager reference](assets/archive-replay-pager-stable-2026-08-31.png): route `/`, exact `1440 × 1000` browser viewport, fixture-collected Every national park replay showing the `2 of 2 · Collected in U.S. National Parks` pager in its stable details slot before the independently scrollable long-title, long-note memory body, captured 2026-08-31. This supersedes the former trailing placement in `archive-replay-pager-2026-08-27.png`, which remains historical evidence.
...
- [Badge adjustment reference](assets/studio-badge-adjustment-2026-09-02.png): route `/#studio/starter:visited-yosemite`, exact `1440 × 1000` CSS viewport, fixture-only Yosemite opened from Discover and adjusted in session to a wool-armband shield with the tag `granite`, showing the header's badge name and live `Save adjustments`, the image panel, the appearance panel on its adjusted values, and the live preview rendering them, captured 2026-09-02. This supersedes the candidate-comparison and publishing surfaces in `studio-manufactured-source-art-2026-08-25.jpg` and `studio-image-replacement-2026-08-28.jpg`, which remain historical evidence of the removed creation workbench.
- [Sticky construction bench reference](assets/studio-sticky-bench-2026-09-02.png): the same route, viewport, and session scrolled to the end of the panels, showing the badge and its reset actions still in view beside Tags, Quote, and Collections, captured 2026-09-02. Before this the bench scrolled away and left half the page empty while the owner changed a shape they could no longer see.
- [Badge adjustment floor reference](assets/studio-badge-adjustment-320-2026-09-02.png): the same route at the exact `320 × 700` CSS viewport floor, fixture-only Yosemite on its catalogue default, showing the mark and badge name on one header row with `Back to Discover` and the save action on the next, captured 2026-09-02. Before this all three landed in one row on top of each other.
...
The 2026-09-02 badge-adjustment sweep inspected Discover, a badge's page, and Badge Studio at `1440 × 900`, `1024 × 768`, `375 × 812`, the exact `320 × 720` floor, and `720 × 400` short landscape. No document overflowed horizontally at any of the fifteen states and no browser console reported an error. Looking at the rendered result rather than the tests found three defects the passing suite did not: the construction bench scrolled away and left half the desktop page empty, the mark, badge name and both actions collided in one header row at the floor, and the tag field's row flex-basis became a `220px` height once the entry stacked. A separate flow opened Studio cold on `#studio/<recordId>` in a clean profile, which exposed that the archive only opened from the Archive surface's suspended effect, and then adjusted, saved, returned and activated, which exposed that the re-shown Archive still held the badge as it was before the save and refused activation. Both are now gated. The completed flow also confirmed forced `?fallback` keeps Front, Edge and Back inspection of the adjusted shield, and that a collected badge's face stays inert under a forced click while tags and collections still save.

The 2026-08-31 same-set navigation sweep compared short and wrapped preparation titles across the `1 of 50` to `10 of 50` digit transition and short versus long-title, long-note replay memories at `1440 × 1000`, `768 × 1024`, `320 × 800`, `960 × 390`, and `844 × 390`. Both `44 × 44px` step rectangles remained pixel-identical within every viewport in live rendering and in the repeated forced-fallback branches, compact documents retained equal client and scroll extents, the tablet overlay remained exactly one viewport tall, and the browser reported zero errors. Record changes reset only the named, keyboard-focusable memory body and move focus from a changing set action to that visible region; preparation opened from a scrolled grid at document offset zero, kept the complete short-landscape header visible, and reached Back to set then Previous through ordinary forward Tab. At the `320px` floor, the fixed preparation rectangles are `(14, 127, 44, 44)` and `(262, 127, 44, 44)`, while replay's are `(23, 359, 44, 44)` and `(253, 359, 44, 44)`; the pre-repair controls moved `37px` in preparation and more than `455px` in replay.
