...
The Archive presentation layer exposes Collection, Timeline, and Discover as ephemeral sections. Timeline is a pure earned-record projection over the loaded `ArchiveState`: it adds no repository, migration, provider call, or durable navigation field; it uses only record-owned occurrence and activation data plus the exact activation-pinned source and render recipe. Discover is a pure tracked-catalogue projection independent from `ArchiveState`: available cards target the four existing record IDs, selected-study cards carry no record target, and search or filtering changes no durable state. The shared renderer composes a recipe-driven static badge preview for every Timeline card without allocating a Canvas or WebGL context, and one ephemeral record ID permits at most one card to replace its preview with the live 3D or renderer-independent fallback viewer; opening an entry selects that same record in Collection. Repeat occurrences remain a separate future domain and persistence decision.
...
The current read-only Discover catalogue is deliberately weaker than an admitted runtime catalogue and visually labels that distinction. Its safe projection contains exactly `66` unique visualized concepts: four available starters and sixty-two additional park studies after Yosemite deduplication. Only an `available` entry carries an existing starter record ID and may switch to Collection; a `source-study` entry has only a bounded preview and `Not yet published` status. Search and availability filters are ephemeral, and discovery changes no record, pack, saying, backup, or Studio state.
