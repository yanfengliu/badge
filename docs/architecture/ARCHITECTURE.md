...
The 2026-08-23 foundation implements that workspace, the two builds and separate remembered localhost origins, separate IndexedDB repositories, inward-facing contract packages, deterministic fixture art, a PNG-only canonical pack compiler and hostile dependency-closed admission module, reproducible and independently admitted starter catalogue and theme pack bytes, self-contained backup of the current Archive records and exact fully decoded earned visual bytes, qualified-identity refusal before restore, state-preserving corrupt-source repair, and a shared React Three Fiber renderer candidate. Durable Archive source images are restricted to strict PNG. Studio and tracked developer fixtures may use WebP or JPEG before publication or deterministic local derivation normalizes their decoded pixels to PNG outside Git. It does not yet implement installed-pack storage or ledgers, complete backup closure for arbitrary installed but unearned packs, Studio project backup or durable release history, `.badgebrief` UI, provider adapters, service workers, the renderer stress/performance gate, or later roadmap scope.
...
IndexedDB is scoped to the full browser origin, including port, so local startup treats the selected Archive and Studio port pair as durable data identity rather than rotating to arbitrary Vite ports.

The launcher prefers Archive at `http://127.0.0.1:4173` and Studio at `http://127.0.0.1:4174`. When no machine-local origin record exists and unrelated software owns either preferred port, it finds one adjacent free pair, prints an explicit warning that any prior-origin data remains at its old address, and exclusively records the selected pair in ignored `.badge-local/ports.json`. Their disposable development fixtures remain at `http://127.0.0.1:5173` and `http://127.0.0.1:5174` respectively and must not be presented as durable data.

Every later start probes both remembered origins. A complete existing Badge pair is reused, a missing companion is started on its reserved adjacent port, and an unrelated listener on either remembered port produces an actionable refusal instead of an origin change. A listener that times out before proving its Badge identity remains explicitly unidentified and also blocks relocation; slow but valid Badge startup has a bounded two-second identity window. Archive-to-Studio and Studio-to-Archive links derive the companion from that adjacent pair. The machine-local record owns only addresses, never application data, and malformed records are preserved rather than replaced.

The ports `4175` and `4176` remain reserved for the optional Studio provider and Archive saying companions, while `5173` and `5174` remain disposable fixture origins; dynamic pair selection skips all four and remembered records containing any of them are preserved and refused as invalid. Each companion accepts only the exact remembered origin of its owning application and requires a separate short-lived unguessable launch capability. Neither service is a cross-application API or filesystem bridge, and compromise of one capability grants no authority over the other service or database.
...
Deliberate relocation requires explicit backup, restore, or another migration mechanism before the remembered pair changes. Separate HTML identity markers, entry points, Vite configs, output directories, CSPs, service-worker scopes, and persistence namespaces prevent one build or an unrelated HTTP listener from masquerading as the other.
