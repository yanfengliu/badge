...
The 2026-08-25 foundation implements that workspace, the two route-scoped builds on one remembered localhost origin, separate IndexedDB repositories, inward-facing contract packages, four starter records seeded with source-checked historical quotations, one-action quote-only regeneration through prompt v3 and quotation contract v3, exact minimal-payload contracts, a live same-listener Claude Code adapter with a no-model-call first-use disclosure review, deterministic fixture art, a PNG-only canonical pack compiler and hostile dependency-closed admission module, reproducible and independently admitted starter catalogue and theme pack bytes, self-contained backup of the current Archive records and exact fully decoded earned visual bytes, qualified-identity refusal before restore, state-preserving corrupt-source repair, and a shared React Three Fiber renderer candidate. Durable Archive source images are restricted to strict PNG. Studio and tracked developer fixtures may use WebP or JPEG before publication or deterministic local derivation normalizes their decoded pixels to PNG outside Git. It does not yet implement installed-pack storage or ledgers, a general installed-definition quotation-bank contract, complete backup closure for arbitrary installed but unearned packs, Studio project backup or durable release history, `.badgebrief` UI, live art-provider adapters, service workers, the renderer stress/performance gate, or later roadmap scope.

The same foundation now includes a pure Studio-only `catalogue-authoring` package with the official NPS `National Parks (63)` edition and exact Census/FIPS `U.S. States (50)` edition, `113` selected source-study manifests, three candidate plans per concept, a `24`-revision creator-neutral style registry used across the state primary selections, deterministic exact source-art prompt compilation, source-linked historical-figure defaults for the state studies, and a `57`-idea common-achievement bank. These records are authoring inputs rather than admitted definitions or published visuals, and Phase 3 publication remains incomplete. Archive imports none of that authoring code; its read-only Discover section consumes a separately checked closed `catalogue-fixtures/discovery` projection plus exactly the `113` integrity-bound list thumbnails, exposing `116` unique visualized concepts across four sets after Yosemite deduplication without adding personal records or runtime badge capabilities.

The Archive presentation layer exposes Collection, Timeline, and Discover as ephemeral sections, with Badge Studio linked as the fourth primary destination to its separately built `/studio/` application. Collection is a pure earned-record projection over loaded `ArchiveState`; it creates a default-closed shelf only for a set represented by at least one earned record, while qualified membership mapping, search, expansion state, and derived statistics add no repository, migration, provider call, or durable navigation field. Timeline is another earned-record projection using only record-owned occurrence and activation data plus the exact activation-pinned source and render recipe. Discover combines the closed tracked-catalogue projection with an earned-ID overlay from `ArchiveState`: available cards target the four existing record IDs, selected-study cards carry no record target, search and set selection operate over all `116` concepts, and the card surface renders only an initial `24` matches before explicit progressive reveal. A replay overlay owns one ephemeral earned record ID and exposes exact occurrence and activation times, frozen quote and provenance, note, and every qualified set membership; choosing a mapped set filters Discover. The shared renderer composes recipe-driven static previews without allocating a Canvas or WebGL context, while replay alone mounts the passive single-turn viewer and Timeline permits at most one live inspector. Repeat occurrences remain a separate future domain and persistence decision.
...
The local route host mounts the independently built Studio entry point at `/studio/`, but the Archive bundle has no Studio lazy chunk, database access, visual-generation port, art prompt recipe, art-provider SDK, or art-provider endpoint. Studio transfers finished work to Archive only through a closed, data-only published pack that Archive independently validates and installs. Archive may explicitly export a minimal data-only authoring request for Studio, but neither application reads the other's database or receives the other's private state. The separately bounded Archive quotation-selection port does not weaken this visual-authoring boundary.
...
`archive-web → catalogue-fixtures/discovery` for the closed tracked discovery projection only; the projection has no dependency on `catalogue-authoring`, Studio, persistence, packs, quotations, renderer recipes, providers, or personal state
...
Selected source studies are not `PublishedBadgeVisual` values and their full assets, prompt records, candidate plans, quotation authoring records, and provenance do not cross into Archive. A closed Git-tracked projection of stable concept identity, title, criterion, place, accessible description, and qualified reviewed-thumbnail key may cross into Archive Discover, together with only the integrity-bound `128 × 128` derivative, as a non-actionable catalogue preview. The complete parks and states releases still need render recipes and fallback closure for every entry, strict publish-safe normalization, canonical pack compilation, independent admission, authenticated quotation banks, and explicit installation before any study can enter Collection, planning, activation, Timeline, quotation regeneration, or 3D inspection; the parks release additionally needs selected all-parks composite art.

The `63` selected `896 × 896` JPEG studies are each at or below `256 KiB` and total exactly `15,211,909` bytes, about `14.51 MiB`, as tracked authoring inputs. Their `63` deterministic `128 × 128` list derivatives total `373,657` bytes with an `8,004`-byte maximum; Studio lists and Archive Discover use only those derivatives, Studio alone reserves one full source for the selected detail, and the Archive production bundle gate requires the exact derivative count, aggregate bytes, and per-file ceiling. The Studio all-list worst-case decoded-pixel bound is therefore one `896 × 896` image plus sixty-three `128 × 128` images, `7,340,032` bytes, about `7.00 MiB`; Archive Discover never decodes the full selected source. That repository and browser result does not establish runtime-pack capacity: strict-PNG admission charges decoded scanline or RGBA size, so direct use of all `63` studies at `896 × 896` would charge about `193 MiB` before the composite or additional maps, above the current `64 MiB` aggregate cap.
...
The state campaign adds `50` tracked `896 × 896` JPEG studies at or below the ordinary `256 KiB` binary ceiling and `50` deterministic `128 × 128` list derivatives. A System.Drawing gate fully decodes and probes every state and park source plus every list derivative instead of trusting a JPEG size marker. The exact Archive discovery manifest binds `113` qualified thumbnails across parks and states totaling `682,741` bytes; production emits them beneath `assets/discovery/<catalogue>/<slug>-<hash>.jpg`, then rejects any missing, changed, swapped, additional, oversized, header-only, or full-size authoring asset against that exact qualified key. These repository measurements establish bounded list media only and do not establish a runtime state-pack quality tier or decoded-image admission result.
...
Planning-only expansion records live behind non-production catalogue-fixture exports and are forbidden from Archive and Studio production imports. They bind later research scope to `281` incorporated Washington municipalities, `483` incorporated California cities and towns, PBS's fixed 2018 _Great American Read_ `100`, a non-exhaustive `34`-place GO TOKYO-informed editorial seed whose item-level authority URLs remain an authoring gate, one generic user-entered Michelin dining milestone with no redistributed restaurant directory or brand assets pending permission or licensing, and `64` optional non-normative life-milestone prompts. A planning record has no Discover identity, selected art, quotation, definition, pack, lifecycle state, provider action, or personal record.
...
The current Discover catalogue is deliberately weaker than an admitted runtime catalogue and visually labels that distinction. Its safe projection contains exactly `116` unique visualized concepts plus a closed four-set registry: four published starters, sixty-two additional park studies after Yosemite deduplication, and fifty state studies. Every entry carries one or more set IDs; only a published entry carries an existing starter record ID and may be prepared or activated, while a `source-study` entry has only a bounded preview and `Not yet published` status. The UI overlays earned record IDs without copying personal state into catalogue fixtures, renders earned cards in full color and all potential cards in grey with explicit text, derives `x / y collected` from the complete set population, searches and filters the complete projection, and reveals card results in explicit batches of `24`. Collection separately renders only represented earned sets and never manufactures empty shelves from this complete registry. Search, set filters, and reveal count are ephemeral, and discovery changes no record, pack, saying, backup, or Studio state except through the existing explicit published-badge preparation and activation flow.
