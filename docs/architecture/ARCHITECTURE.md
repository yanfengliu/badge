...
Lifecycle, browser, and terminal verification never use the canonical interactive record. The runner admits a branded task-owned target confined to an ignored `tmp/local-startup/` path, cleans its owned record and listener on success and failure, and leaves an absent or legitimate pre-existing canonical `.badge-local/site.json` byte-for-byte untouched; repository gates therefore never require the canonical record to be absent. A legacy `.badge-local/ports.json` is preserved and refused by normal startup rather than being reinterpreted as the single-site record. The explicit `npm run recover:legacy` command reads that record without modifying it, serializes startup with an owned transient lock, reopens the separate builds with pair-aware links on the exact old origins, and does not create or change `.badge-local/site.json`; this keeps old Studio data reachable while Studio backup remains roadmap work.
...
Local-startup contracts cover one-port selection across the valid non-reserved range, exact and mutually exclusive Archive and Studio route identity, Studio document deep-link routing, free, Badge, unrelated, unresolved, incomplete, and route-swapped listeners, remembered-origin continuity, malformed and prior-version record preservation, concurrent first publication, read-during-publication invisibility, interrupted-publication and listener-cleanup failure, and terminal listener disposal. Verification-target contracts admit only branded paths under ignored `tmp/local-startup/`, exercise absent and sentinel canonical state, and prove exact canonical-state invariance; a source-boundary check proves executable verification can call only the verification-target factory and never references `.badge-local`. Legacy-recovery contracts cover prior-validator port parity, unreadable and malformed evidence, one-site-host refusal, exact pair-aware link injection, serialized competing startup, partial-lock cleanup, partial-start rollback, every owned close, surfaced cleanup failure, and separation from the new site record.
