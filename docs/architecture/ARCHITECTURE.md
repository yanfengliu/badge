...
Selected source studies are not `PublishedBadgeVisual` values and their full assets, prompt records, candidate plans, and provenance do not cross into Archive. A closed Git-tracked projection of stable concept identity, title, criterion, place, accessible description, and reviewed thumbnail filename may cross into Archive Discover, together with only the integrity-bound `128 × 128` derivative, as a non-actionable catalogue preview. The complete parks release still needs selected all-parks composite art, a render recipe and fallback closure for every entry, strict publish-safe normalization, canonical pack compilation, independent admission, an authenticated quotation bank, and explicit installation before any study can enter Collection, planning, activation, Timeline, or 3D inspection.
...
The current Discover catalogue is deliberately weaker than an admitted runtime catalogue and visually labels that distinction. Its safe projection contains exactly `66` unique visualized concepts plus a closed canonical set registry: four published starters and sixty-two additional park studies after Yosemite deduplication. Every entry carries one or more set IDs; only a published entry carries an existing starter record ID and may be prepared or activated, while a `source-study` entry has only a bounded preview and `Not yet published` status. The UI overlays earned record IDs without copying personal state into catalogue fixtures, renders earned cards in full color and all potential cards in grey with explicit text, and derives `x / y collected` from the complete set population. Search and set filters are ephemeral, and discovery changes no record, pack, saying, backup, or Studio state except through the existing explicit published-badge preparation and activation flow.
