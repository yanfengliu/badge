...
`AchievementRecord.acceptedSaying` is the only durable saying field in the current slice. Model-written and direct text persist as the exact normalized accepted string; accepting a source-checked historical-quotation proposal persists `“text” — Person, Source` in that same field so existing string-shaped records require no migration. An accepted string in an explicit quote-and-attribution form is never preloaded into the personal-text editor: replacing it is a named action that starts blank and leaves the attributed string untouched until new personal text is saved, including after reload or backup restore.

A structured pending proposal, its model provenance, its quotation kind, and any source URL are ephemeral Archive application and UI state. A regenerated passage is a pending proposal, not an in-place mutation; accepting a proposal or editing the text is an explicit user action, and neither operation changes the selected art or appearance.

Accepting updates only the record's string field; there is no separate durable source, kind, or saying-update timestamp yet. An unaccepted proposal never becomes the displayed saying by accident, and adding structured accepted-saying provenance later requires a versioned schema change and migration rather than inference from formatted or directly entered text.
...
A separate Archive saying-generation port accepts only the badge title, criterion, optional saying-specific direction, optional bounded source-checked historical quotations, request ID, and cancellation signal, then returns either a proposed model-written paragraph or one hydrated exact quotation plus provenance. Each allowed quotation contains an opaque ID, exact text, person, source title, and HTTPS source URL; the raw model response may select only the ID, while the adapter resolves all displayed quotation content from that exact request. The application boundary then revalidates the hydrated quote against the originating request so a buggy or future provider cannot substitute an ID, words, person, source title, or URL. Adding description or any other field requires a new explicit outbound-data decision and disclosure surface; the port cannot import Studio or art-generation code.
...
Saying-specific direction is a closed optional object containing curated non-personal theme cues, voice, variation, and deliberately supplied user direction, with the exact field and total-payload ceilings in `docs/design/product-spec.md`. User direction may contain personal text and therefore remains part of the visible outbound disclosure. The optional quotation shortlist is also closed, bounded, source-checked catalogue data and is displayed exactly in the review. The internal port request carries request identity and cancellation state, but those values are never serialized into the model prompt. The adapter owns the exact versioned system and user prompt recorded in the product spec; it never derives prompt context from personal notes, dates, occurrences, accepted sayings, visibility, art, or other Archive state.

`saying-contract` is the provider-neutral executable boundary for prompt v2: it canonicalizes and caps the outbound JSON at `12 KiB`, strictly admits bounded closed provider JSON, validates model-written paragraphs or exact supplied quotation IDs, resolves quotations from request data rather than model prose, binds complete hydrated quotation metadata to the originating shortlist, rejects unsafe Unicode controls and invisible-only content, redacts raw malformed output from public errors, and carries provider, model, prompt-version, and generation-time provenance without credentials. Its quotation-contract version covers the executable quotation record grammar, strictness, uniqueness, HTTPS-source rule, and semantic validation in addition to the separately disclosed field names and limits. `saying-live-contract` closes the current same-origin protocol around it by pinning Claude Code, `claude-sonnet-4-6`, the exact disclosure fingerprint, a `16 KiB` route-body ceiling, response sizes and errors, and literal live provenance.
...
Normalize model-written proposals and direct input by trimming outer whitespace and collapsing internal whitespace, including line breaks, to one space, yielding one logical paragraph. The prompt targets one to three sentences without a word-count limit; the executable contract intentionally does not pretend that punctuation segmentation can prove a universal sentence count. Model-written and supplied quotation text stops at `600` Unicode grapheme clusters, `2,048` code points, and `7,680` UTF-8 bytes; in the historical-selection branch the model returns only an ID and does not author or modify quotation text. Accepted persisted strings stop at `800` graphemes, `3,072` code points, and `10,240` UTF-8 bytes with a cheap `6,144`-UTF-16-unit preflight before normalization or grapheme segmentation. Directly entered quotations remain ordinary user-authored text and must include their own quotation marks and attribution. Never silently truncate stored or displayed text.

Tests and fixture browser runs use separate deterministic Studio art candidates plus in-process Archive model-written-style paragraphs and source-checked historical quotations so ordinary verification does not spend model budget or require network access; fixture mode mounts no saying HTTP endpoint.

Live Studio adapters disclose the destination and exact outbound visual fields before media leaves the device. The live Archive saying adapter discloses the destination, provider, pinned model, exact system prompt v2, exact canonical text values and optional quotation shortlist, complete allowed field scope, quotation-contract version, and excluded private fields before its first request in a page session. Its in-memory acknowledgment is fingerprinted over the disclosed contract and is invalidated by any provider, destination, model, prompt, field scope, quotation-contract version, normalization, or limit change; every executable quotation-record schema change must bump that version. Later explicit requests may carry different per-achievement title, criterion, direction, and shortlist values under that acknowledged scope. It excludes description, notes, dates, occurrence data, accepted sayings, visibility, art, and unrelated draft fields; closing and navigation send nothing; manual writing always works without Claude.
...
Archive Playwright starts from `/` on the remembered Badge site and covers installation of a deterministic prepublished pack and the Yosemite acceptance journey with no Studio controls: real pointer rotation, native engaged-only wheel zoom and disengaged page-scroll release, light adjustment, keyboard equivalents, context-loss recovery, forced first-run and post-restore WebGL initialization failure with zero capability-probe calls, equal-height front, edge, and back fallback views, original and source-linked historical-quotation proposals, saying retry and direct editing, prompt-v2 provider disclosure, multiline paste, over-limit validation, narrow-layout paragraph wrapping, activation reload safety, composite completion, and Archive restore into a clean profile. Network interception proves browsing, inspection, and activation never request visual generation.
