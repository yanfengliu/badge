...
`AchievementRecord.acceptedSaying` remains the only durable saying-content field in the current slice, accompanied by an opaque `quotationRevision` concurrency token that carries no personal or provider-bound content. Every fresh starter record stores its designated curated quotation as `“text” — Person, Source`; a compatible existing unearned record whose field is still `null` receives only that default through an idempotent compare-and-swap initialization transaction, and a non-null legacy or accepted value is never overwritten or relabeled during startup. An earned record with no quotation remains readable evidence but is incompatible with the truthful starter contract: initialization never invents a quote for it, startup routes it to state rescue, and normal restore plus replacement recovery refuse it before mutation.
...
There is still no durable quotation ID, HTTPS source URL, or saying-update timestamp. The starter UI recovers the reviewed source link only when the complete accepted string exactly matches one curated fixture record, while legacy strings remain readable without receiving verified-source semantics. A general installed-pack quotation bank and structured durable selected identity require a later versioned contract and migration rather than inference from formatted text.
...
A separate Archive quotation-regeneration port accepts only the badge title, criterion, optional saying-specific direction, a required bounded list of source-checked historical quotations, request ID, and cancellation signal, then returns one hydrated exact quotation plus provenance. Each allowed quotation contains an opaque ID, exact text, person, source title, and HTTPS source URL; curated release review establishes the person's historicity and the record's authentic wording and attribution, while the current starter bank is locked by an exact-value manifest test. The raw model response is the closed object `{ quotationId }`, while the adapter resolves all displayed quotation content from that exact request. The controller retains a private immutable parsed request, gives the provider a disposable clone, and revalidates the hydrated quote against the retained request so provider mutation cannot substitute an ID, words, person, source title, or URL. The repository then rejects caller-invented lists by admitting the same result only against its construction-time record-bound bank. Adding description or any other field requires a new explicit outbound-data decision and disclosure surface; the port cannot import Studio or art-generation code.
