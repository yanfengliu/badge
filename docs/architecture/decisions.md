...
The owner required that any two badges not use the same quote. The previous catalogue selected only `16` distinct texts across `350` seeded badges, producing `11,266` colliding unordered default pairs, because D-045 and D-055 required record binding but treated uniqueness as local to one request or set-level bank. The replacement `catalogue-quotation-data` release owns exactly two source-checked historical quotations for every current definition. All `700` stable IDs and canonical text keys are globally disjoint, every one of the `350` designated defaults is unique, and the fixture generator projects those private banks verbatim into `badge.catalogue.discovery@1.0.0-alpha.3` and the four starter records. A symmetric close-match scan plus an independently adjudicated retirement ledger additionally keeps spelling, word-order, excerpt, and translation variants of the same quotation on only one badge. The dependency-neutral `quotation-identity` package defines the one NFKD, default-ignorable deletion, fixed-point uppercase/lowercase caseless expansion, combining-mark removal, non-letter/non-number collapse, and trim operation used by the catalogue and Archive saying contract; quotation contract v6 adds request-local canonical-key injectivity and rotates the live disclosure fingerprint.

`npm run audit:quotation-sources` is the explicit network-capable provenance gate for that release. It groups all `700` records by their `30` unique source URLs, verifies each source's exact byte SHA-256, and requires every associated quotation to occur case- and punctuation-exactly after leading-byte-order-mark and whitespace normalization. Verified bytes are cached under ignored `tmp/catalogue-quotation-source-cache/`; a cache miss or corrupt entry falls back to the tracked URL, while `--offline` requires a complete valid cache. This manual gate is deliberately outside ordinary offline `npm run verify`.

Repository enforcement is transactional rather than trusting curated data alone. A quote update or activation compares the proposed canonical text with every other nonempty accepted saying, using the exact trusted quotation when available, every plausible quote prefix from a legacy formatted string, and the complete string for raw legacy prose before the same IndexedDB transaction writes state or source bytes. Concurrent windows therefore permit only one claimant. Restore and recovery without current defaults reject any collision involving an unearned record; with defaults, initialization, restore, and recovery reserve every earned or activated accepted value first, preserve sealed records byte-for-byte even when old history contains duplicates, then process unearned records in stable order and keep the trusted current value, designated default, or first trusted alternate only when free. Exhaustion refuses atomically without state, object, or quarantine writes.

This supersedes D-057's shared Shakespeare designation for the fifty game records and D-055's set-level-bank publication rule while retaining their visual, media, source, reconciliation, and earned-history boundaries. It extends D-045's exact-source and no-earned-rewrite rules: historical duplicates already sealed before this release remain truthful evidence, but no future update, activation, restore, recovery, or seeded catalogue state may add an unearned duplicate.
