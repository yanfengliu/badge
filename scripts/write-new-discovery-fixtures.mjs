...
try {
  const [books, education, dining, styles] = await Promise.all([
    ...
    server.ssrLoadModule("/packages/catalogue-authoring/src/art-styles.ts"),
  ]);
  const catalogue = { ...books, ...education, ...dining, ...styles };
  const expectedCounts = {
    bookAuthoringRecords: 50,
    educationMilestoneAuthoringRecords: 2,
    michelinDiningAuthoringRecords: 132,
  };
  ...
  const bookRows = catalogue.bookAuthoringRecords.map((record) => {
    const style = catalogue.findArtStyle(record.candidateStyles[0]);
    if (!style) {
      throw new Error(
        `Book ${record.definitionId} references unknown primary style ${JSON.stringify(record.candidateStyles[0])}.`,
      );
    }
    return row(
      record,
      `Selected ${style.label} badge study for ${record.title}: ${record.artBrief.description}`,
    );
  });
  const educationRows = catalogue.educationMilestoneAuthoringRecords.map((record) =>
    row(record, record.accessibleDescription),
  );
  const diningRowsByRegion = Object.fromEntries(
    ["bay-area", "new-york-city", "washington-dc"].map((regionId) => [
      regionId,
      catalogue.michelinDiningAuthoringRecords
        .filter((record) => record.restaurant.regionId === regionId)
        .map((record) =>
          row(
            record,
            record.accessibleDescription,
