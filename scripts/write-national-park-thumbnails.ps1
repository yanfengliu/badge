...
# Historical filename retained for compatibility; this implements catalogue-list-thumbnail@1 for any normalized catalogue.
...
if ($null -eq $jpegCodec) {
  throw "Catalogue thumbnail generation could not find the Windows JPEG encoder. Install a System.Drawing-compatible JPEG codec and retry."
}
...
if ($sourceFiles.Count -ne $ExpectedCount) {
  throw "Catalogue thumbnail generation found $($sourceFiles.Count) source studies in $SourceDirectory; expected exactly $ExpectedCount before deriving list art."
}
...
foreach ($sourceFile in $sourceFiles) {
  ...
  if (-not $outputFile.StartsWith("$OutputDirectory$([IO.Path]::DirectorySeparatorChar)", [StringComparison]::OrdinalIgnoreCase)) {
      throw "Catalogue thumbnail generation refused output path $outputFile because it escapes $OutputDirectory."
  }
  ...
  try {
    ...
    if ($source.Width -ne 896 -or $source.Height -ne 896) {
      throw "Catalogue thumbnail generation rejected $($sourceFile.FullName) at $($source.Width) by $($source.Height); normalize the selected study to 896 by 896 first."
    }
    ...
    try {
      ...
      if ($stream.Length -gt 16KB) {
        throw "Catalogue thumbnail generation produced $($sourceFile.Name) at $($stream.Length) bytes; revise recipe catalogue-list-thumbnail@1 before committing it."
      }
